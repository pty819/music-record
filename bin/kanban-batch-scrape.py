...
# 调用 MiniMax Anthropic 兼容接口，原生分离 thinking 和 summary
...
def summarize_cn(excerpt, artist_album, tags_raw_str=""):
    ...
    try:
        ...
        # Anthropic endpoint natively separates thinking and content:
        # - First block: type=thinking (we ignore it)
        # - Last block: type=text (this is our summary)
        result = ""
        for block in reversed(message.content):
            if hasattr(block, 'type') and block.type == 'text' and hasattr(block, 'text'):
                ...
                break
        if result:
            # Extract from <summary> tag as fallback / double guarantee
            import re
