...
description: 每日巡检前卫/实验/学院派爵士/电子/世界音乐评论站，输出结构化专辑推荐清单。RSS 优先，Playwright 保底。每日 cron 输出到 Telegram。
category: music
...
output_target: telegram（推送到 Home Channel）
---
...
Step 8: 推送到 Telegram Home Channel
...
> JSON 版保存在本地 `~/.hermes/cron/output/daily_album_recs_YYYY_MM_DD.json`，不推送到 Telegram。
...
if result.returncode != 0 and "already exists" not in result.stderr:
    ...
    # 仓库已存在则直接克隆
    subprocess.run(
        ["gh", "repo", "clone", "pty819/音乐推荐", REPO_PATH],
        capture_output=True
    )

# 初始化 git（如果 clone 下来的目录为空）
...
### ⚠️ 重要：所有有效音乐都要写入 markdown

不论评分高低，**所有经过评分的候选专辑都要写入当日的 markdown**，格式如下：
...
| 4 | Album | Artist | Site | Type | 9 | 不足主推荐标准 |
| 5 | Album | Artist | Site | Type | 8 | 候选补充 |
...
```
