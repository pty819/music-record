#!/usr/bin/env python3
"""Scrape Boomkat — 2026-06-28 swarm.

Reuses the extract JS from bin/scrape_boomkat.py but with longer waits so the
CF Turnstile can clear before we pull the listing DOM.
"""
...
def wait_for_page(tab_id: str, want_products: int, max_wait: int = 30) -> bool:
    """Poll the page until it shows at least want_products items, or CF cleared."""
...
def main():
    ...
    sys.stderr.write("Sleeping 5s before polling to let CF start...\n")
    time.sleep(5)
    ...
    try:
        ...
        if not wait_for_page(tab_id, want_products=10, max_wait=45):
            ...
            if cf_blocked:
                ...
                with open("/home/liyifan/music-record/2026/06/2026-06-30/boomkat_reviews.json", "w") as f:
