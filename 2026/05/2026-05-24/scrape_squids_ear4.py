#!/usr/bin/env python3
"""Scrape The Squid's Ear via AJAX endpoint + date-based cutoff.

Strategy:
- Fetch page 0 from AJAX endpoint (most recent ~2637 items, highest newsIDs)
- Batch-fetch detail pages for first N items to find date cutoff
- Stop scanning once all items are older than 3 days
"""
...
BATCH      = 20  # parallel detail fetches
...
def fetch(url, timeout=20):
    ...
    res = subprocess.run(
        ['curl', '-sL', '--max-time', str(timeout),
         ...
         url],
        capture_output=True, text=True, errors='replace')   # handle Latin-1 bytes
    return res.stdout
...
def main():
    # Fetch only page 0 (most recent ~2637 items)
    log("Fetching listing page 0...")
    html = fetch("https://www.squidco.com/ear/returnArticlesV2.php?start=0")
    rows = parse_table_page(html)
    log(f"Found {len(rows)} rows on page 0")

    # Sort by news_id descending (newest first)
    rows.sort(key=lambda r: int(r['news_id']), reverse=True)
    log(f"newsID range: {rows[0]['news_id']} – {rows[-1]['news_id']}")

    # Batch-scan dates to find cutoff
    results = []
    cutoff_found = False
    batches = 0

    # Process in batches of BATCH, scanning until we find items older than cutoff
    i = 0
    while i < len(rows):
        batch_rows = rows[i:i+BATCH]
        with ThreadPoolExecutor(max_workers=BATCH) as ex:
            futures = {ex.submit(get_date, r['url']): r for r in batch_rows}
        ...
        if cutoff_found:
            log("Cutoff found — older than 3 days")
            break
